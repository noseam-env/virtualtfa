...
// 4 bytes
void virtual_tfa_util_write_int32_le(char* buf, int32_t value) {
...
// 4 bytes
int32_t virtual_tfa_util_read_int32_le(const char* buf) {
...
// 4 bytes
void virtual_tfa_util_write_uint32_le(char* buf, uint32_t value) {
    buf[0] = (char) (value & 0xFF);
    buf[1] = (char) ((value >> 8) & 0xFF);
    buf[2] = (char) ((value >> 16) & 0xFF);
    buf[3] = (char) ((value >> 24) & 0xFF);
...
// 4 bytes
uint32_t virtual_tfa_util_read_uint32_le(const char* buf) {
...
// 8 bytes
void virtual_tfa_util_write_uint64_le(char* buf, uint64_t value) {
    buf[0] = (char) (value & 0xFF);
    buf[1] = (char) ((value >> 8) & 0xFF);
    buf[2] = (char) ((value >> 16) & 0xFF);
    buf[3] = (char) ((value >> 24) & 0xFF);
    buf[4] = (char) ((value >> 32) & 0xFF);
    buf[5] = (char) ((value >> 40) & 0xFF);
    buf[6] = (char) ((value >> 48) & 0xFF);
    buf[7] = (char) ((value >> 56) & 0xFF);
}

// 8 bytes
uint64_t virtual_tfa_util_read_uint64_le(const char* buf) {
...
void
virtual_tfa_input_stream_set_read_function(virtual_tfa_input_stream* this, virtual_tfa_read_function read_function) {
...
void
virtual_tfa_input_stream_set_close_function(virtual_tfa_input_stream* this, virtual_tfa_close_function close_function) {
...
int virtual_tfa_input_stream_read(virtual_tfa_input_stream* this,
                                  char* buffer,
...
void virtual_tfa_entry_set_input_stream_supplier(virtual_tfa_entry* this,
                                                 virtual_tfa_input_stream_supplier stream_supplier) {
...
void virtual_tfa_archive_add(virtual_tfa_archive* this, virtual_tfa_entry* entry) {
    this->entries_size++;
    virtual_tfa_entry** new_entries = (virtual_tfa_entry**) realloc(this->entries,
                                                                    this->entries_size * sizeof(virtual_tfa_entry*));
    if (!new_entries) {
...
const char virtual_tfa_magic[4] = {'t', 'f', 'a', '1'};

void virtual_tfa_util_set_magic(tfa_header* header) {
    memcpy(header->magic, virtual_tfa_magic, sizeof(virtual_tfa_magic));
}

void virtual_tfa_util_set_mode(tfa_header* header, tfa_mode_t mode) {
    virtual_tfa_util_write_int32_le(header->mode, mode);
}

void virtual_tfa_util_set_ctime_mtime(tfa_header* header, tfa_utime_t ctime, tfa_utime_t mtime) {
    virtual_tfa_util_write_uint64_le(header->ctime, ctime);
    virtual_tfa_util_write_uint64_le(header->mtime, mtime);
}

void virtual_tfa_util_set_namesize(tfa_header* header, tfa_namesize_t namesize) {
    virtual_tfa_util_write_uint32_le(header->namesize, namesize);
}

void virtual_tfa_util_set_filesize(tfa_header* header, tfa_size_t filesize) {
    virtual_tfa_util_write_uint32_le(header->filesize, filesize);
}
...
tfa_header* virtual_tfa_util_convert_entry_to_header(virtual_tfa_entry* entry) {
    ...
    if (header) {
        virtual_tfa_util_set_magic(header);
        virtual_tfa_util_set_mode(header, entry->mode);
        virtual_tfa_util_set_ctime_mtime(header, entry->ctime, entry->mtime);
        virtual_tfa_util_set_namesize(header, (tfa_namesize_t) strlen(entry->name) + 1); // last \0
        virtual_tfa_util_set_filesize(header, entry->size);
    }
    return header;
}
...
int virtual_tfa_writer_write(virtual_tfa_writer* this,
                             char* buffer,
                             tfa_size_t buffer_size,
    ...
    for (int i = 0; i < this->archive->entries_size; ++i) {
        ...
        if (virtual_tfa_util_calc_part_write(buffer_size_left,
            ...
            int read_result = virtual_tfa_input_stream_read(this->current_stream, buffer + bytes_written,
...
virtual_tfa_file_info virtual_tfa_util_file_info_constructor(const char* name,
                                                             tfa_size_t filesize,
                                                             tfa_utime_t ctime,
                                                             tfa_utime_t mtime) {
    virtual_tfa_file_info fileinfo;
    fileinfo.name = name;
...
const char* virtual_tfa_util_delimiter = "/";

bool virtual_tfa_util_is_path_valid(const char* path) {
    char* str = strdup(path);
    char* token = strtok(str, virtual_tfa_util_delimiter);
    ...
    while (token != NULL) {
        ...
        token = strtok(NULL, virtual_tfa_util_delimiter);
    }
    ...
    while (buffer_size_left > 0) {
        ...
        if (this->_cur_remainHeaderSize > 0) {
            ...
            if (this->_cur_remainHeaderSize == 0) {
                ...
                this->_cur_h_mode = (tfa_mode_t) virtual_tfa_util_read_int32_le(header.mode);

                this->_cur_h_ctime = virtual_tfa_util_read_uint64_le(header.ctime);
                this->_cur_h_mtime = virtual_tfa_util_read_uint64_le(header.mtime);

                this->_cur_remainNameSize = this->_cur_h_namesize = virtual_tfa_util_read_uint32_le(
                        header.namesize);
                this->_cur_remainFileSize = this->_cur_h_filesize = virtual_tfa_util_read_uint64_le(
                        header.filesize);
